...
it('advances anchor day one through regular months and across years', function (): void {
    ...
    $february = $cycle->nextDueOn(1, dateOnly('2026-01-01'));
    $march = $cycle->nextDueOn(1, $february);
    $january = $cycle->nextDueOn(1, dateOnly('2026-12-01'));
...
it('clamps anchor day thirty one in February without drifting in March', function (): void {
    ...
    $february = $cycle->nextDueOn(31, dateOnly('2026-01-31'));
    $march = $cycle->nextDueOn(31, $february);
...
it('clamps anchor day thirty one to leap day and then returns to thirty one', function (): void {
    ...
    $february = $cycle->nextDueOn(31, dateOnly('2024-01-31'));
    $march = $cycle->nextDueOn(31, $february);
...
it('clamps anchor day thirty in February and returns to thirty in March', function (): void {
    ...
    $february = $cycle->nextDueOn(30, dateOnly('2026-01-30'));
    $march = $cycle->nextDueOn(30, $february);
    $leapFebruary = $cycle->nextDueOn(30, dateOnly('2024-01-30'));
    $leapMarch = $cycle->nextDueOn(30, $leapFebruary);
...
it('handles anchor day twenty nine in leap and non leap years', function (): void {
    ...
    $nonLeapFebruary = $cycle->nextDueOn(29, dateOnly('2026-01-29'));
    $nonLeapMarch = $cycle->nextDueOn(29, $nonLeapFebruary);
    $leapFebruary = $cycle->nextDueOn(29, dateOnly('2024-01-29'));
    $leapMarch = $cycle->nextDueOn(29, $leapFebruary);
...
it('rejects invalid anchor days', function (): void {
    ...
    expect(fn () => $cycle->nextDueOn(0, dateOnly('2026-01-01')))
        ->toThrow(InvalidArgumentException::class)
        ->and(fn () => $cycle->nextDueOn(32, dateOnly('2026-01-01')))
        ->toThrow(InvalidArgumentException::class);
});
