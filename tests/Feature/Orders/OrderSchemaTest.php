...
it('creates PostgreSQL check constraints and the open dine-in partial unique index', function (): void {
    ...
    $constraints = collect(DB::select(<<<'SQL'
        ...
        from pg_constraint
        where conrelid in ('orders'::regclass, 'order_subtables'::regclass)
        SQL))
