...
active user in the current tenant, assigned to the current branch, and holding
the existing `orders.take` permission. The Orders Application action validates
that rule through `Identity\Contracts\UserDirectory`; UI adapters only present
the branch-scoped staff list and call the action. Clearing a waiter remains a
valid null assignment.
...
same capability that allows a user to work in the order workspace.
